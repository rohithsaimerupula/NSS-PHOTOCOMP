const app = {
    ...
    renderHome: () => `
        <section id="hero" class="animate" style="text-align: center; padding: 5rem 0 3rem;">
            <div class="hero-badge">🌿 NSS Photo Competition 2026</div>
            <h1 style="font-size: 3.8rem; margin-bottom: 1.25rem; line-height: 1.1;">
                <span class="accent-text">Urban Green</span><br>Pockets
            </h1>
    ...
    renderRegister: () => `
        ...
        <div class="glass-card animate" style="max-width: 800px; margin: 2rem auto; position: relative; z-index: 10;">
            ...
            <form id="registration-form" onsubmit="app.handleRegister(event)">
                ...
                <button type="submit" class="btn btn-primary" style="width: 100%; margin-top: 1rem; padding: 1rem;">🌿 Submit for Approval</button>
            </form>
        </div>
    `,
    ...
    renderLogin: () => `
        ...
        <div class="glass-card animate" style="max-width: 420px; margin: 4rem auto; position: relative; z-index: 10;">
            ...
            <form onsubmit="app.handleLogin(event)">
                <div class="form-group">
                    <label>Team Lead Register No.</label>
                    <input type="text" id="login-reg" required placeholder="e.g. CSE2024001">
                </div>
                <div class="form-group">
                    <label>Password</label>
                    <input type="password" id="login-pass" required placeholder="Your team password">
                </div>
                <button type="submit" class="btn btn-primary" style="width: 100%; padding: 1rem;">🌿 Access Upload Portal</button>
            </form>
            <p style="text-align:center; margin-top:1rem; font-size:0.85rem; color: var(--text-muted);">Not registered yet? <span style="color:var(--accent); cursor:pointer;" onclick="app.showPage('register')">Enrol your team →</span></p>
        </div>
    `,
        ...
        <div class="glass-card animate" style="max-width: 420px; margin: 4rem auto; position: relative; z-index: 10;">
            ...
            <form onsubmit="app.handleAdminLogin(event)">
                <div class="form-group">
                    ...
                    <input type="text" id="admin-user" required placeholder="Staff username">
                </div>
                <div class="form-group">
                    <label>Password</label>
                    <input type="password" id="admin-pass" required placeholder="Staff password">
                </div>
                <button type="submit" class="btn btn-primary" style="width: 100%; padding: 1rem; background: linear-gradient(135deg, var(--accent-earth), var(--accent-warm));">🌿 Login as NSS Staff</button>
            </form>
        </div>
    `,
        ...
        return `
            <div class="animate" style="margin: 2rem 0;">
                ...
                <div class="glass-card" style="margin-top: 2rem;">
                    <h3 style="margin-bottom: 0.5rem; color: var(--accent-warm);">⚙️ Account Settings</h3>
                    <p style="margin-bottom: 1.5rem; font-size: 0.9rem; color: var(--text-muted);">Update your NSS staff login credentials below.</p>
                    <form onsubmit="app.handleUpdateAdmin(event)" style="max-width: 400px;">
                        ...
                        <button type="submit" class="btn btn-primary" style="background: linear-gradient(135deg, var(--accent-earth), var(--accent-warm));">🔒 Update Credentials</button>
                    </form>
                </div>
            </div>
        `;
        ...
        return `
            <div class="glass-card animate" style="max-width: 620px; margin: 4rem auto;">
                ...
                <div style="background: rgba(116,196,118,0.07); border: 1px solid rgba(116,196,118,0.2); border-radius: 12px; padding: 1rem 1.25rem; margin-bottom: 2rem;">
                    <p style="font-size: 0.85rem; color: var(--error); font-weight: 600; margin-bottom: 0.4rem;">⚠️ Strict Competition Rules:</p>
                    <ul style="font-size: 0.83rem; color: var(--text-muted); padding-left: 1.2rem; line-height: 1.8;">
                        <li>Photos must be <strong>original</strong> and taken by your team.</li>
                        <li><strong>Smartphone cameras ONLY</strong> — No DSLRs or digital cameras.</li>
                        <li><strong>No AI-generated</strong> or heavily edited images.</li>
                        <li>Subject must relate to <strong>Urban Green Pockets</strong> theme.</li>
                    </ul>
                </div>

                <form onsubmit="app.handleUpload(event)">
                    <div class="form-group">
                        <label>📷 Urban Green Photo (Max 10MB)</label>
                        <input type="file" id="comp-photo" accept="image/*" required>
                    </div>
                    <div class="form-group">
                        <label>📄 EXIF / Photo Info Screenshot (Verification)</label>
                        <p style="font-size: 0.8rem; color: var(--text-muted); margin-bottom: 0.5rem;">Upload a screenshot of your image details to verify it was taken on a smartphone.</p>
                        <input type="file" id="info-screenshot" accept="image/*" required>
                    </div>
                    <div class="form-group">
                        <label>🌿 Photo Title</label>
                        <input type="text" id="photo-title" required placeholder="e.g. The Forgotten Garden">
                    </div>
                    <button type="submit" class="btn btn-primary" style="width: 100%; padding: 1rem;">🌱 Submit My Entry</button>
                </form>
            </div>
        `;
    },
    ...
    renderGallery: async () => {
        const entries = await storage.getEntries();
        return `
            <div class="animate" style="padding: 4rem 0;">
                <div style="text-align: center; margin-bottom: 3rem;">
                    <div class="hero-badge" style="display:inline-flex; margin-bottom: 1rem;">🌿 Urban Green Pockets 2026</div>
                    <h2 style="font-size: 2.8rem; color: var(--accent);">Photo Gallery</h2>
                    <p style="color: var(--text-muted); font-size: 1rem; max-width: 600px; margin: 0.75rem auto 0;">A collection of urban green spaces discovered and captured by our participants.</p>
                </div>
                ${entries.length === 0 ? `
                    <div style="text-align:center; padding: 4rem 0;">
                        <span style="font-size: 3rem;">🌿</span>
                        <p style="color: var(--text-muted); margin-top: 1rem; font-size: 1.05rem;">The gallery is empty for now. It will bloom once submissions arrive!</p>
                    </div>
                ` : `
                    <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 1.5rem; max-width: 1100px; margin: 0 auto;">
                        ${entries.map(entry => `
                            <div class="glass-card" style="padding: 0; overflow: hidden; cursor:pointer;" onclick="app.showImageModal('${entry.title} — by ${entry.leadName}', '${entry.photoPath}')">
                                <img src="${entry.photoPath}" alt="${entry.title}" style="width: 100%; height: 220px; object-fit: cover; display:block; transition: transform 0.4s ease;" onmouseover="this.style.transform='scale(1.04)'" onmouseout="this.style.transform='scale(1)'">
                                <div style="padding: 1rem 1.25rem;">
                                    <h4 style="color: var(--accent); margin-bottom: 0.3rem; font-size:1rem;">🌿 ${entry.title}</h4>
                                    <p style="font-size: 0.82rem; color: var(--text-muted);">by ${entry.leadName}</p>
                                </div>
                            </div>
                        `).join('')}
                    </div>
